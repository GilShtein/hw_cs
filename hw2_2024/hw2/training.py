...
class Trainer(abc.ABC):
    ...
    def fit(
        ...
        **kw,
        ...
        for epoch in range(num_epochs):
            ...
            #     if early_stopping and epochs_no_improvement >= early_stopping:
            #         print(f"Early stopping at epoch {epoch+1}", verbose)
            ...
            if best_acc is None or test_result.accuracy > best_acc:
                ...
                if early_stopping is not None and early_stopping == epochs_without_improvement:
                    print("There is no improvement of the test accuracy. Stopping early.")
                    actual_num_epochs = epoch + 1
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        # Return BatchResult
        return BatchResult(loss.item(), correct_pred)
        '''
...
class ClassifierTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        num_correct = torch.sum(predicted == y).item()
        # ========================
...
class LayerTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        # ====== YOUR CODE: ======
        #  - Forward pass
        outputs = self.model(X.reshape(X.shape[0], -1)) # X is in ths shape [batch, 3, image_size[0], image_size[1], we need to flatten the image to be in the shape [batch_size, 3*image_size[0]*image_size[1]] to pass it through the network. The result is the class scores
        #outputs = self.model(X)
        ...
        # Clear gradients w.r.t. parameters
        self.optimizer.zero_grad()
        curr_layer_grad = self.loss_fn.backward(loss) # We have to the grad of each layer to be the input to the backward of the model
        #  - Backward pass
        self.model.backward(curr_layer_grad)  
        #  - Optimize params
        self.optimizer.step()
        #  - Calculate number of correct predictions (make sure it's an int,
        #    not a tensor) as num_correct.
    ...
    def fit(
        ...
        **kw,
        ...
        for epoch in range(num_epochs):
            ...
            train_loss.append(torch.mean(torch.tensor(train_result.losses)).item())
            train_acc.append(train_result.accuracy)
            train_loss.append(torch.mean(torch.tensor(test_result.losses)).item())
            test_acc.append(test_result.accuracy)
...
class LayerTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        #    not a tensor) as num_correct.
        # ====== YOUR CODE: ======
        outputs = self.model(X.reshape(X.shape[0], -1)) 
         # X is in ths shape [batch, 3, image_size[0], image_size[1], we need to flatten the image to be in the shape[batch_size,3*image_size[0]*image_size[1]] to pass it through the network. The result is the class scores
