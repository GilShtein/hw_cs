...
class MLP(nn.Module):
    ...
    def __init__(
        ...
        # ====== YOUR CODE: ======
        super().__init__()
        entry_layer = nn.Linear(in_features=self.in_dim, out_features=dims[0])
        entry_activation = ACTIVATIONS[nonlins[0]]() if isinstance(nonlins[0], str) else nonlins[0]
        model_layers = [entry_layer, entry_activation]

        for idx in range(1, len(dims)):
            current_activation = ACTIVATIONS[nonlins[idx]]() if isinstance(nonlins[idx], str) else nonlins[idx]
            model_layers.extend([
                nn.Linear(in_features=dims[idx - 1], out_features=dims[idx]),
                current_activation
            ])

        self.network_layers = nn.Sequential(*model_layers)
        # ========================
