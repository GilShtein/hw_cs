...
class ReLU(LeakyReLU):
    ...
    def __init__(self):
        # ====== YOUR CODE: ======
        super(ReLU, self).__init__(alpha = 0.0)
...
class Linear(Layer):
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        dw = dout.T @ x  # Gradient w.r.t. W
        db = dout.sum(dim=0)  # Gradient w.r.t. b
        dx = dout @ self.w  # Gradient w.r.t. x
        
        # Store gradients in the parameters
        self.dw += dw
        self.db += db
...
class CrossEntropyLoss(Layer):
    ...
    def forward(self, x, y):
        ...
        true_class_score = x.gather(1, y.unsqueeze(1)).squeeze(1)
        loss_per_example = -true_class_score + torch.log(torch.sum(torch.exp(x), dim=1))
    ...
    def backward(self, dout=1.0):
        ...
        dx = softmax_logits.clone().detach()
...
class Dropout(Layer):
    ...
    def forward(self, x, **kw):
        ...
        if self.training_mode:  
            binom = torch.distributions.binomial.Binomial(probs=1 - self.p)
            self.mask = binom.sample(x.size()) * (1.0 / (1 - self.p))
            out = x * self.mask
        else:  
            out = x
        # ========================
...
class Sequential(Layer):
    ...
    def forward(self, x, **kw):
        ...
        # ====== YOUR CODE: ======
        out = self.layers[0](x, **kw)
        # output of the first layer pass to the next layer and so on
    ...
    def backward(self, dout):
        ...
        for layer in self.layers[::-1]:
        # Backpropagate through this layer and update 'din' to be the gradient
    ...
    def params(self):
        ...
        for layer in self.layers:
            # Assuming each layer has a params() method that returns its parameters
        ...
        **kw,
        ...
        for i in range(len(all_dims) - 1):
            ...
            if i < ( len(all_dims) - 2):
                ...
                # Apply non-linearity and dropout except for the last layer
                if dropout > 0:
...
class Linear(Layer):
    ...
    def backward(self, dout):
        ...
        dx = dout @ self.w
        # using the chain rule, the gradient of L with respect to 𝑊
        # dL/dw = dL/dy * dy/dw
        # dy/dw = x
        self.dw += dout.T @ x
        # using the chain rule for b
        # dL/db = dL/dy * dy/db
        # dy/db = 1
...
class CrossEntropyLoss(Layer):
    ...
    def forward(self, x, y):
        ...
        # Ensure y is of type LongTensor and reshaped to (N,)
        
        true_class_score = x[range(N), y]

'''
'''
        # Initialize an empty tensor to store true_class_score
        true_class_score = torch.zeros(N, dtype=x.dtype, device=x.device)

        # Compute true_class_score using a loop
        for i in range(N):
            true_class_score[i] = x[i, y[i]]
'''
'''
    ...
    def __init__(
        ...
        **kw,
        ...
        for hidden in hidden_features:
            linear_layer = Linear(prev_features, hidden)
            layers.append(linear_layer)
            # Ensure all parameters in the Linear layer have requires_grad=True
            for param, grad in linear_layer.params():
                param.requires_grad_(True)
            #layers.append(Linear(prev_features, hidden))
            if activation == "relu":
                layers.append(ReLU())
        ...
        # Add the last layer from the last hidden layer to the output layer
        layers.append(Linear(prev_features, num_classes))
        for param, grad in layers[-1].params():
            param.requires_grad_(True)
        # ========================
